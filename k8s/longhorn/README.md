...
| レプリカ数 | 1 (ラボ向け) |
| ノードダウン時 Pod 削除 | delete-both-statefulset-and-deployment-pod |
| 予期せぬ detach 時 Pod 自動削除 | true |
...
**全ワーカーノードでの対処 (`/etc/multipath.conf`):**
...
> この設定は `terraform/main.tf` の `null_resource.k3s_multipath_blacklist` で全ワーカーノードに自動適用される。VM 再構築時も Terraform が自動で設定する。
