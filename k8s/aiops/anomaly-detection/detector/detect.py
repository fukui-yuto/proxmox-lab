...
# エラーログのフィルタ条件 (再利用)
# NOTE: stream.keyword: stderr は除外。多くの Pod が通常ログも stderr に書くため
#       誤検知 (false positive) の原因になる。level フィールドのみで判定する。
ERROR_FILTER = {
    "bool": {
        "should": [
            {"terms": {"level.keyword": ["error", "ERROR", "ERR", "fatal", "FATAL", "critical", "CRITICAL"]}},
            {"terms": {"log_level.keyword": ["error", "ERROR", "fatal", "FATAL"]}},
        ],
