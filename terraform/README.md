...
| k3s-worker08 | pve-node03 | 192.168.210.29 | 2 | local (qcow2) | 8GB |
...
worker03〜05 は pve-node02、worker06〜08 は pve-node03 に直接デプロイする。
...
1. VM / LXC コンテナ作成
2. k3s master / worker03〜08 インストール・クラスター参加
...
locals {
  ...
  worker_ips  = ["192.168.210.24", ..., "192.168.210.29"]
}
```

### worker インストールリソース

`null_resource.k3s_workers_install` (count=6) で worker03〜08 を統一管理。
worker01/02 は削除済み。worker09/10 は統合により削除済み。
- index 0/1/2 → `k3s_worker_node02[0/1/2]` (node02, worker03/04/05)
- index 3/4/5 → `k3s_worker_node03[0/1/2]` (node03, worker06〜08)
