...
worker03 / worker04 は pve-node02 に直接デプロイする (クロスノードクローン)。
テンプレート (vmid 9000) は node01 に置いたまま、`clone.node_name = "pve-node01"` を指定することで
Proxmox API がクロスノードクローンを処理し、disk を node02 の local-lvm に直接作成する。
