...
| k3s-worker10 | pve-node03 | 192.168.210.31 | 2 | local (qcow2) | 4GB |
...
`null_resource.k3s_workers_install` (count=8) で worker03〜10 を統一管理。
worker01/02 は削除済み。
- index 0/1/2 → `k3s_worker_node02[0/1/2]` (node02, worker03/04/05)
- index 3/4/5/6/7 → `k3s_worker_node03[0/1/2/3/4]` (node03, worker06〜10)
