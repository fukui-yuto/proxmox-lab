...
2. **k3s ノード名が `k3s-master` にならない**
   `initialization` ブロックに `hostname = "k3s-master"` が未設定だと、
   テンプレートのホスト名のまま k3s に登録され `kubectl wait node/k3s-master` が失敗する。
   `main.tf:59` に `hostname = "k3s-master"` を追加済み。
