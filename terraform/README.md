...
|---------|--------|-----|-----|-----------|-----|
| k3s-master | pve-node03 | 192.168.210.21 | 2 | local (qcow2) | 6GB |
| k3s-worker09 | pve-node01 | 192.168.210.30 | 1 | data-pve-node01 (ZFS) | 4GB |
| k3s-worker10 | pve-node01 | 192.168.210.31 | 1 | data-pve-node01 (ZFS) | 4GB |
| k3s-worker11 | pve-node01 | 192.168.210.32 | 1 | data-pve-node01 (ZFS) | 4GB |
...
| k3s-worker06 | pve-node03 | 192.168.210.27 | 2 | local (qcow2) | 8GB |
| k3s-worker07 | pve-node03 | 192.168.210.28 | 2 | local (qcow2) | 8GB |
| dns-ct (Pi-hole) | pve-node03 | 192.168.210.53 | - | local | 512MB |
...
worker09〜11 は pve-node01、worker03〜05 は pve-node02、worker06〜07 は pve-node03 に直接デプロイする。
...
1. VM / LXC コンテナ作成
2. k3s master / worker09〜11, worker03〜07 インストール・クラスター参加
...
locals {
  ...
  worker_node01_ips = [".30", ".31", ".32"]  # worker09/10/11
  worker_node02_ips = [".24", ".25", ".26"]  # worker03/04/05
  worker_node03_ips = [".27", ".28"]          # worker06/07
  all_worker_ips = concat(...)
}
```

### worker インストールリソース

`null_resource.k3s_workers_install` (count=8) で全ワーカーを統一管理。
- index 0/1/2 → `k3s_worker_node01[0/1/2]` (node01, worker09/10/11)
- index 3/4/5 → `k3s_worker_node02[0/1/2]` (node02, worker03/04/05)
- index 6/7   → `k3s_worker_node03[0/1]`   (node03, worker06/07)
...
ansible raspberrypi -i inventory/hosts.yml -m shell \
  -a "ssh-keygen -f /root/.ssh/known_hosts -R 192.168.210.21 && \
      ...
      ssh-keygen -f /root/.ssh/known_hosts -R 192.168.210.31 && \
      ssh-keygen -f /root/.ssh/known_hosts -R 192.168.210.32 && \
...
### k3s-master / dns-ct を node03 に移行 + node01 に worker 追加 (実施済み: 2026-04-27)

pve-node01 (i3-5010U 2C4T) で k3s-master が CPU を飽和させ Proxmox API がタイムアウトする問題の対策。

**移行内容:**
- k3s-master (VM 201): pve-node01 → pve-node03 (ZFS → local qcow2 に変換してオフラインマイグレーション)
- dns-ct (CT 101): pve-node01 → pve-node03

**VM 再配置:**
- pve-node01: worker09/10/11 (1vCPU, 4GB × 3台) を新規追加
- pve-node03: worker08 (VM 209) を削除 (k3s-master + dns-ct 移行分のリソース確保)

**`main.tf` の変更内容:**
- `k3s_master`: node_name を `pve-node03`、disk を `local` (qcow2) に変更
- `pihole`: node_name を `pve-node03`、disk を `local` に変更
- `k3s_worker_node01` (count=3) を新規追加 (VM 210〜212)
- `k3s_worker_node03`: count `3` → `2` (worker08 削除)
- `locals`: ノード別 worker IP リストに分離
- `k3s_workers_install`: count `6` → `8`
- `expand_disk_node03`: count `3` → `2`

クラスター合計: 3 vCPU (node01) + 3 vCPU (node02) + 4 vCPU (node03 worker) + 2 vCPU (node03 master) = **12 vCPU**

### pve-node03 ワーカー統合: 5台→3台 + メモリ増強 (実施済み: 2026-04-19)

worker06 が CPU 94% で繰り返し NotReady になり、Prometheus 等がダウン → ArgoCD Degraded の悪循環が発生。
根本原因は pve-node03 (Ryzen 5 3550H 4C/8T, 32GB) 上で 5 VM × 2 vCPU = 10 vCPU の CPU overcommit。

`main.tf` の変更内容:
- `k3s_worker_node03` の count を `5` → `3` (worker09/10 を削除)
- `k3s_worker_node03` のメモリを `4096` → `8192` (4GB → 8GB)
- `worker_ips` から worker09 (192.168.210.30) / worker10 (192.168.210.31) を削除
- `k3s_workers_install` count: `8` → `6`
- `expand_disk_node03` count: `5` → `3`
- レジストリ・sysctl・multipath の IP ループから .30/.31 を削除

クラスター合計: 3 vCPU (node02) + 6 vCPU (node03) = **9 vCPU**、node03 RAM: 3 × 8GB = 24GB (32GB 中)

### worker01 削除 + worker10 追加 + node03 CPU 2コア化 (実施済み: 2026-04-19)

CPU request 飽和 (97%+) と pve-node01 過負荷 (load average 5.85, FS read-only) の構造的対策:

1. **worker01 (node01) 削除**: master + worker + dns-ct が 2C/4T NUC に集中していたため worker を排除
2. **worker10 (node03) 追加**: 削除分の補填 + 容量増
3. **worker06〜10 の CPU 2コア化**: pve-node03 (Ryzen 5 3550H 4C/8T) のリソースを活用

`main.tf` の変更内容:
- `k3s_worker` (node01) リソースを削除 (count 1→0)
- `k3s_worker_node03` の count を `4` → `5`、`cpu.cores` を `1` → `2`
- `worker_ips` から worker01/02 を削除、worker10 (192.168.210.31) を追加
- `k3s_workers_install` のインデックスマッピングを簡略化
- master に NoSchedule taint を k3s install 引数で追加
...
k3s-worker09   Ready    <none>                 3m    v1.34.x+k3s1
k3s-worker10   Ready    <none>                 3m    v1.34.x+k3s1
k3s-worker11   Ready    <none>                 3m    v1.34.x+k3s1
k3s-worker03   Ready    <none>                 3m    v1.34.x+k3s1
k3s-worker04   Ready    <none>                 3m    v1.34.x+k3s1
k3s-worker05   Ready    <none>                 3m    v1.34.x+k3s1
