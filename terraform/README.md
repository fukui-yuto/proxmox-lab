...
| k3s-worker06 | pve-node03 | 192.168.210.27 | local-lvm | 4GB |
| k3s-worker07 | pve-node03 | 192.168.210.28 | local-lvm | 4GB |

> **pve-node03 ハードウェア:** BOSGAME E2 / AMD Ryzen 5 3550H / 32GB RAM
...
worker03〜05 は pve-node02、worker06〜07 は pve-node03 に直接デプロイする。
...
1. VM / LXC コンテナ作成
2. k3s master / worker01〜05 インストール・クラスター参加
...
`null_resource.k3s_workers_install` (count=7) で worker01〜07 を統一管理。
- index 0/1 → `k3s_worker[0/1]` (node01)
- index 2/3/4 → `k3s_worker_node02[0/1/2]` (node02)
- index 5/6 → `k3s_worker_node03[0/1]` (node03)
