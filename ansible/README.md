...
ansible-playbook -i inventory/hosts.yml playbooks/06-proxmox-sdn.yml --tags check

# SDN を自動設定する場合 (WebUI で設定済みなら不要)
ansible-playbook -i inventory/hosts.yml playbooks/06-proxmox-sdn.yml
```
...
```bash
ansible-playbook -i inventory/hosts.yml playbooks/07-resilience.yml
```
